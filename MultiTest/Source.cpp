...
double p1Qvalue[eqSize][eqSize][pqSize][pqSize][ACTION] = { 0 };
double p2Qvalue[eqSize][eqSize][pqSize][pqSize][ACTION] = { 0 };
      ...
      for(int pi= 0; pi <pqSize; pi++ ) {
        for(int pj = 0; pj < pqSize; pj++) {
          for (int k = 0; k < ACTION; k++) {
            p1Qvalue[i][j][pi][pj][k] = 0;
            p2Qvalue[i][j][pi][pj][k] = 0;
          }
        }
      }
    }
...
int getMAXQValue(State afterstate, int playernumber) {
  ...
  if (playernumber == 1) {
    ...
    maxQ = p1Qvalue[afterstate.first][afterstate.second][afterstate.allyfirst][afterstate.allysecond][nextaction];
  }

  if (playernumber == 2) {
    int nextaction = getMaxQAction(afterstate, playernumber);                 //after�ł̍ő�Q�l���o���s��
    maxQ=p2Qvalue[afterstate.first][afterstate.second][afterstate.allyfirst][afterstate.allysecond][nextaction];
  }
  return maxQ;
}
...
int getMaxQAction(State state, int playernum) {
  ...
  if (playernum == 1) {
    for (int i = 0; i < ACTION; i++) {
      double q = p1Qvalue[state.first][state.second][state.allyfirst][state.allysecond][i];
      //�ő�Q�l�ƂȂ��s�����L��
      if (q > maxQ) {
        action = i;
        maxQ = q;
  ...
  if (playernum == 2) {
    for (int i = 0; i < ACTION; i++) {
      double q = p2Qvalue[state.first][state.second][state.allyfirst][state.allysecond][i];
      //�ő�Q�l�ƂȂ��s�����L��
      if (q > maxQ) {
        action = i;
        maxQ = q;
...
bool calcSuccessReward(State state, int action, double maxQ, long double AttenuationAlpha, int playernum) {
  if (playernum == 1) {
    p1Qvalue[state.first][state.second][state.allyfirst][state.allysecond][action] = (1 - AttenuationAlpha)*p1Qvalue[state.first][state.second][state.allyfirst][state.allysecond][action] + AttenuationAlpha* (subrewards + ganna * maxQ);
  }

  if (playernum == 2) {
    p2Qvalue[state.first][state.second][state.allyfirst][state.allysecond][action] = (1 - AttenuationAlpha)*p2Qvalue[state.first][state.second][state.allyfirst][state.allysecond][action] + AttenuationAlpha* (subrewards + ganna * maxQ);
  }
  return true;
}
...
bool calcFinishReward(State state, int action, double maxQ, long double AttenuationAlpha, int playernum) {
  if (playernum == 1) {
    p1Qvalue[state.first][state.second][state.allyfirst][state.allysecond][action] = (1 - AttenuationAlpha)* p1Qvalue[state.first][state.second][state.allyfirst][state.allysecond][action] + AttenuationAlpha* (rewards + ganna * maxQ);
  }

  if (playernum == 2) {
    p2Qvalue[state.first][state.second][state.allyfirst][state.allysecond][action] = (1 - AttenuationAlpha)*p2Qvalue[state.first][state.second][state.allyfirst][state.allysecond][action] + AttenuationAlpha* (rewards + ganna * maxQ);
  }
  return true;
}
...
bool calcFaildReward(State state, int action, double maxQ, long double AttenuationAlpha, int playernum) {
  if (playernum == 1) {
    p1Qvalue[state.first][state.second][state.allyfirst][state.allysecond][action] = (1 - AttenuationAlpha)*p1Qvalue[state.first][state.second][state.allyfirst][state.allysecond][action] + AttenuationAlpha* (faild + ganna * maxQ);
  }

  if (playernum == 2) {
    p2Qvalue[state.first][state.second][state.allyfirst][state.allysecond][action] = (1 - AttenuationAlpha)*p2Qvalue[state.first][state.second][state.allyfirst][state.allysecond][action] + AttenuationAlpha* (faild + ganna * maxQ);
  }
  return true;
}
