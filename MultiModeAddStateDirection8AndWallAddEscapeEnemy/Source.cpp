...
double p1Qvalue[qSize][qSize][PDIRECTION][wallstateMAX][qSize][ACTION] = { 0 };
double p2Qvalue[qSize][qSize][PDIRECTION][wallstateMAX][qSize][ACTION] = { 0 };
  ...
  for (int i = 0; i < qSize; i++) {
    for (int j = 0; j < qSize; j++) {
      for (int n = 0; n < PDIRECTION; n++) {
        for (int wall = 0; wall < wallstateMAX; wall++) {
          for (int m = 0; m < qSize; m++) {
            for (int k = 0; k < ACTION; k++) {
              p1Qvalue[i][j][n][wall][m][k] = 0.0;
              p2Qvalue[i][j][n][wall][m][k] = 0.0;
            }
          }
        }
      }
...
int getMAXQValue(State afterstate, int playernumber) {
  ...
  if (playernumber == 1) {
    ...
    maxQ = p1Qvalue[afterstate.first][afterstate.second][afterstate.pdirection][afterstate.wallstate][afterstate.locate_enemy_count][nextaction];
  }
...
int MultiQlearningMethod(State p1, State p2, State enemy, int gamecount)
{
  ...
  while (episodecount < EPISODECOUNT) {
    ...
    if (flag_wallstate == false) {
      p1state.wallstate = 0;
      p2state.wallstate = 0;
      p1afterstate.wallstate = 0;
      p2afterstate.wallstate = 0;
    }
...
int searchWallState(State player){
  std::bitset<8> bs("00000000");
...
int getMaxQAction(State state, int playernum) {
  ...
  if (playernum == 1) {
    for (int i = 0; i < ACTION; i++) {
      double q = p1Qvalue[state.first][state.second][state.pdirection][state.wallstate][state.locate_enemy_count][i];
      //�ő�Q�l�ƂȂ��s�����L��
      if (q > maxQ) {
        action = i;
        maxQ = q;
  ...
  if (playernum == 2) {
    for (int i = 0; i < ACTION; i++) {
      double q = p2Qvalue[state.first][state.second][state.pdirection][state.wallstate][state.locate_enemy_count][i];
      //�ő�Q�l�ƂȂ��s�����L��
      if (q > maxQ) {
        action = i;
        maxQ = q;
...
bool calcSuccessReward(State state, int action, double maxQ, long double AttenuationAlpha, int playernum) {
  if (playernum == 1) {
    p1Qvalue[state.first][state.second][state.pdirection][state.wallstate][state.locate_enemy_count][action] = (1 - AttenuationAlpha)*p1Qvalue[state.first][state.second][state.pdirection][state.wallstate][state.locate_enemy_count][action] + AttenuationAlpha* (subrewards + ganna * maxQ);
  }

  if (playernum == 2) {
    p2Qvalue[state.first][state.second][state.pdirection][state.wallstate][state.locate_enemy_count][action] = (1 - AttenuationAlpha)*p2Qvalue[state.first][state.second][state.pdirection][state.wallstate][state.locate_enemy_count][action] + AttenuationAlpha* (subrewards + ganna * maxQ);
  }
  return true;
}
...
bool calcFinishReward(State state, int action, double maxQ, long double AttenuationAlpha, int playernum) {
  if (playernum == 1) {
    p1Qvalue[state.first][state.second][state.pdirection][state.wallstate][state.locate_enemy_count][action] = (1 - AttenuationAlpha)* p1Qvalue[state.first][state.second][state.pdirection][state.wallstate][state.locate_enemy_count][action] + AttenuationAlpha* (rewards + ganna * maxQ);
  }

  if (playernum == 2) {
    p2Qvalue[state.first][state.second][state.pdirection][state.wallstate][state.locate_enemy_count][action] = (1 - AttenuationAlpha)*p2Qvalue[state.first][state.second][state.pdirection][state.wallstate][state.locate_enemy_count][action] + AttenuationAlpha* (rewards + ganna * maxQ);
  }
  return true;
}
...
bool calcFaildReward(State state, int action, double maxQ, long double AttenuationAlpha, int playernum) {
  if (playernum == 1) {
    p1Qvalue[state.first][state.second][state.pdirection][state.wallstate][state.locate_enemy_count][action] = (1 - AttenuationAlpha)*p1Qvalue[state.first][state.second][state.pdirection][state.wallstate][state.locate_enemy_count][action] + AttenuationAlpha* (faild + ganna * maxQ);
  }

  if (playernum == 2) {
    p2Qvalue[state.first][state.second][state.pdirection][state.wallstate][state.locate_enemy_count][action] = (1 - AttenuationAlpha)*p2Qvalue[state.first][state.second][state.pdirection][state.wallstate][state.locate_enemy_count][action] + AttenuationAlpha* (faild + ganna * maxQ);
  }
  return true;
}
