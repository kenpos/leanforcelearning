...
State initState(int x, int y) {
        ...
        tmp.first = x;
        tmp.second = y;
        tmp.allyfirst = p_esight;
        tmp.allysecond = p_esight;
        tmp.pterncount = 0;
...
bool calcFinishReward(State state, int action, double maxQ, long double AttenuationAlpha, int playernum) {
        if (playernum == 1) {
                double old_val = p1Qvalue[state.first][state.second][state.allyfirst][state.allysecond][state.pterncount][state.allyactionhistory[0]][state.allyactionhistory[1]][action];
                p1Qvalue[state.first][state.second][state.allyfirst][state.allysecond][state.pterncount][state.allyactionhistory[0]][state.allyactionhistory[1]][action] =
                        (1 - AttenuationAlpha)* old_val + AttenuationAlpha * (rewards + ganna * maxQ);
        }

        if (playernum == 2) {
                double old_val = p2Qvalue[state.first][state.second][state.allyfirst][state.allysecond][state.pterncount][state.allyactionhistory[0]][state.allyactionhistory[1]][action];
                p2Qvalue[state.first][state.second][state.allyfirst][state.allysecond][state.pterncount][state.allyactionhistory[0]][state.allyactionhistory[1]][action] =
                        (1 - AttenuationAlpha)* old_val + AttenuationAlpha * (rewards + ganna * maxQ);
        }
        return true;
}
