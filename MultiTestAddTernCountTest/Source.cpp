...
auto makerandom = std::bind(std::uniform_int_distribution<int>(0, 100), std::mt19937(static_cast<unsigned int>(time(nullptr))));
...
double getMAXQValue(State afterstate, int playernumber) {
        ...
        if (playernumber == 1) {
                ...
                maxQ = p1Qvalue[afterstate.first][afterstate.second][afterstate.allyfirst][afterstate.allysecond][afterstate.pterncount][action];
        }

        if (playernumber == 2) {
                //int nextaction = getMaxQAction(afterstate, playernumber);                 //after�ł̍ő�Q�l���o���s��
                maxQ=p2Qvalue[afterstate.first][afterstate.second][afterstate.allyfirst][afterstate.allysecond][afterstate.pterncount][action];
        }
        return maxQ;
}
...
int MultiQlearningMethod(State p1, State p2, State enemy, int gamecount)
{
        ...
        while (episodecount < EPISODECOUNT) {
                ...
                //cout <<"p1afterstate " << p1afterstate.first <<","<<p1afterstate.second << "," << p1afterstate.allyfirst <<"," << //p1afterstate.allysecond <<"," <<p1afterstate.pterncount << endl;
                //cout <<"p2afterstate " << p2afterstate.first <<","<<p2afterstate.second << "," << p2afterstate.allyfirst <<"," << //p2afterstate.allysecond <<","<< p2afterstate.pterncount << endl;
...
bool calcFinishReward(State state, int action, double maxQ, long double AttenuationAlpha, int playernum) {
        if (playernum == 1) {
                double old_val = p1Qvalue[state.first][state.second][state.allyfirst][state.allysecond][state.pterncount][action];
        ...
        if (playernum == 2) {
                double old_val = p2Qvalue[state.first][state.second][state.allyfirst][state.allysecond][state.pterncount][action];
...
bool calcFaildReward(State state, int action, double maxQ, long double AttenuationAlpha, int playernum) {
        if (playernum == 1) {
                double old_val = p1Qvalue[state.first][state.second][state.allyfirst][state.allysecond][state.pterncount][action];
                p1Qvalue[state.first][state.second][state.allyfirst][state.allysecond][state.pterncount][action] =
                        (1 - AttenuationAlpha)* old_val + AttenuationAlpha * (faildrewards + ganna * maxQ);
        }
